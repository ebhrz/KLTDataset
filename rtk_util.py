...
def get_wls_pnt_pos(o,nav,exsatids=[], SYS = ['G','C','R','E'],SNR = 0, EL = 0, RESD = 10000):
    maxiter = 20
    if o.n < 4:
    ...
    for ii in range(o.n):
        ...
        if  pii == 0 or obsd.sat in exsatids or sysname.ptr[0] not in SYS or obsd.SNR[0] < SNR*1e3:
...
def get_nlos_wls_pnt_pos(o,nav,nlos,SYS = ['G','C','R','E'],exsatids=[]):
    maxiter = 10
    if o.n < 4:
        return {"status":False,"pos":np.array([0,0,0,0]),"msg":"no enough observations","data":{}}
    rs,noeph,dts,var = get_sat_pos(o.data,o.n,nav)
    ...
    for ii in range(o.n):
        ...
        pii = obsd.P[0] #only process L1
        prl.satno2id(obsd.sat,sysname)
        if  pii == 0 or obsd.sat in exsatids or sysname.ptr[0] not in SYS:
            exclude.append(ii-skip)
            prs.append(0)
            sats.append(obsd.sat)
            continue
